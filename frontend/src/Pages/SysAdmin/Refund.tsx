...
type RefundStatus = 'pending' | 'confirmed' | 'cancelled';
...
const statusRefund: Record<RefundStatus, number> = {pending: 1, confirmed: 2, cancelled: 3};
const sortedRefunds = [...orders].sort((a, b) => {
    return statusRefund[a.status] - statusRefund[b.status];
});

const pendingRefund: Refund[] = sortedRefunds.filter(refund => refund.status === 'pending');
const confirmedRefund: Refund[] = sortedRefunds.filter(refund => refund.status === 'confirmed');
const cancelledRefund: Refund[] = sortedRefunds.filter(refund => refund.status === 'cancelled');
...
const Refunds = () => {
    ...
    const rows = [...(showPending ? pendingRefund : []), ...(showConfirmed ? confirmedRefund : []), ...(showCancelled ? cancelledRefund : [])].map((item, index) => (
        <Table.Row
    ...
    return (
        <SysAdminPageContainer>
            <Heading as="h1" size="lg" mb={4}>Refunds List</Heading>
